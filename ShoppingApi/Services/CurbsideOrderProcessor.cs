...
namespace ShoppingApi.Services
{
    public class CurbsideOrderProcessor : BackgroundService
    {
        ...
        private readonly IHubContext<CurbsideHub> _hub;

        public CurbsideOrderProcessor(ILogger<CurbsideOrderProcessor> logger, CurbsideChannel channel, IServiceProvider serviceProvider, IHubContext<CurbsideHub> hub)
        {
            _logger = logger;
            _channel = channel;
           ...
           await foreach(var order in _channel.ReadAllAsync())
            {
                ...
                else
                {
                    ...
                    for(var t = 0; t<numberOfItems; t++)
                    {
                        ...
                        {
                            await _hub.Clients.Client(order.ConnectionId).SendAsync("ItemProcessed", new { message = $"Processed item {t + 1 } of your order", orderId = order.ReservationId });
                        }
                    }
